...
export function VisibilityScopePicker({
  ...
  useEffect(() => {
    if (!open) return;
    const base = allowUsers ? value : { ...value, user_ids: [] };
  ...
  const openRoleChildren = (role: string) => {
    setActiveRole(role);
    setQuery("");
  };
          ...
          <div className="grid min-h-[420px] grid-cols-1 md:grid-cols-[1.1fr_0.9fr]">
            <div className="border-b border-[var(--line)] p-4 md:border-b-0 md:border-r">
              ...
              <div className="max-h-[330px] space-y-1 overflow-auto pr-1">
                  ...
                  <div key={item.role} className="flex items-center gap-2 rounded-[var(--r-sm)] px-2 py-2 hover:bg-[var(--surface-alt)]">
                    ...
                    {allowUsers && (
                      <button
                        ...
                        onClick={() => openRoleChildren(item.role)}
                      >
                ...
                {(activeRole || roleRows.length === 0) && (!allowUsers || userRows.length === 0) && (
                  <div className="px-2 py-8 text-center text-sm text-[var(--text-mute)]">
                    {activeRole ? "这个范围下暂无成员" : "没有匹配结果"}
                  </div>
                )}
              </div>
            </div>
