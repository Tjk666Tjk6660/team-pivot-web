...
# ---- 自动定位 team-pivot-web 代码仓库 ----
# 这些脚本住在 <repo>/scripts/migration-prod/ 下,往上两级就是 repo 根
...
if [[ ! -f "$SCRIPT_DIR/env.init" ]]; then
    ...
    echo "   先 cp env.init.example env.init 并按需修改"
    exit 1
fi
...
# PROD_DB_PATH 允许留空(走 --no-users-db),不在必填里
for v in WORKSPACE BACKUP_DIR SERVICE_NAME EXPECTED_BRANCH \
...
# ---- 派生路径 ----
mkdir -p "$BACKUP_DIR"
export REPORT_DRYRUN="$BACKUP_DIR/migration-report-prod-dryrun.json"
export REPORT_APPLY="$BACKUP_DIR/migration-report-prod-apply.json"

# ---- DB_FLAG ----
# 迁移脚本通过 SQLite URI mode=ro 只读打开 PROD_DB_PATH,
# 详见 scripts/migrate_index_schema.py::_ReadOnlyUserView。
