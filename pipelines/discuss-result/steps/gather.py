"""Gather all posts of a thread for RESULT generation (with author-permission check)."""
...
def main():
    ...
    if not category or not thread:
        sys.stderr.write("gather: missing required params: category and thread\n")
        sys.exit(1)
    ...
    if author and author != ctx.user_id:
        raise PermissionError(
            f"Only the thread author ({author}) can generate a result for this discussion; "
