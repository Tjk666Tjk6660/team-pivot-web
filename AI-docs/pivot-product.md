...
- `index` 不应依赖对 MD 正文的反向解析来更新
- 正确的主流程应当是：客户端或 AI 助手先准备好结构化字段，再通过 API 一次性提交
- 服务端根据这份结构化提交，同时写入 `index` 和对应的 MD 文件

也就是说：

- `index` 不是从 MD 里“猜出来”的
- `index` 是和 MD 一起被“写出来”的

这条原则尤其适用于：

- `verify` 的 `verifications`
- `result` 的 `outcome`
- 各类文件触发的 `status_change`

这些字段都应当由 API 显式提交，而不是依赖服务端去解析正文段落名或正文内容后再反推出结构化结果。

对应地，AI 在这里最合适的角色是：

- 先帮助用户生成或整理正文内容
- 再把其中需要结构化提交的信息整理成 API 字段
- 提交前把这些结构化字段展示给用户确认
- 用户确认后，再由客户端一次性调用 API
...
`act` 的正文不需要承载复杂状态，而应保持为一份清晰的责任说明。下面只是推荐写法，不是强制格式：
...
- 上面的正文段落名只是推荐写法，系统不依赖这些段落名做结构判断
...
更合理的写法是：在正文中直接展开一个 `Verifications` 段，逐条对应 `verifications` 中的每一个 `act`，并给出判断结果与简短评价。但这仍然只是推荐写法，不是强制格式。
...
- 真正的硬约束在 `index` 的 `verifications` 字段里，而不是正文段落名本身
...
更进一步地说，这五种文件类型的正文都不应再被系统强制要求固定模板。正文模板最多只应作为推荐写法、AI 默认生成格式或前端编辑器初始模板存在，而不应成为真正的结构化约束。
