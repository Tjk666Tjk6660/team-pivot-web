...
| 区域 | 文件数 | 复杂度 | 状态 |
|---|---|---|---|
| A. 用户身份层（新模块） | ~11 | 高 | ✅ 已完成（Tasks 3-7） |
| A. 现有模块重构 | ~10 | 中 | 🟡 进行中（Tasks 9-11 已落，Tasks 12-15 待开始） |
| A. 鉴权切换 | 3 | 低 | ⏳ 待开始（Task 15） |
| B. 迁移脚本 + 单测 | 2 | 中 | ✅ 已完成（Task 8） |
| C. 前端新页面 + 组件 | ~9 | 中-高 | ⏳ 待开始（Tasks 23-29） |
| C. 前端现有修改 | ~6 | 低-中 | ⏳ 待开始（Tasks 22 / 30-31） |
| D. 单测 + 集成测 | ~10 | 中 | 🟡 进行中（伴随每条 task 落地） |
| E. 配置 + 文档 | 4 | 低 | 🟡 进行中（Task 33 收尾） |

按 task 状态推进，不写工期承诺；review checkpoint 控制节奏。
