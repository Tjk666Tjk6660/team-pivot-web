...
def test_rejects_invalid_type(drafts):
    with pytest.raises(ValueError):
        drafts.create(pivot_user_id="ou_1", type_="unknown", body_md="x")


def test_list_for_user_returns_only_owned(drafts):
    a = drafts.create(pivot_user_id="ou_a", type_="proposal", body_md="a")
    drafts.create(pivot_user_id="ou_b", type_="proposal", body_md="b")
...
def test_list_sorted_by_updated_desc(drafts):
    import time as _t
    a = drafts.create(pivot_user_id="ou_a", type_="proposal", body_md="first")
    _t.sleep(0.01)
    b = drafts.create(pivot_user_id="ou_a", type_="proposal", body_md="second")
    items = drafts.list_for_user("ou_a")
...
def test_update_patches_only_given_fields(drafts):
    d = drafts.create(
        pivot_user_id="ou_1", type_="proposal",
...
def test_update_empty_returns_current(drafts):
    d = drafts.create(pivot_user_id="ou_1", type_="proposal", body_md="x")
