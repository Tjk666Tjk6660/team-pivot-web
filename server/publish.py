...
def publish_matter_create(
    ...
    *,
    ...
    title: str,
    initial_item: dict,
    matter_owner_pivot_user_id: str | None = None,
    notifier: Notifier | None = None,
    ...
    `matter_owner_pivot_user_id` (matter-level owner, distinct from item-level
    ...
    if matter_owner_pivot_user_id and matter_owner_pivot_user_id != user.open_id:
        matter_owner_pinyin = _resolve_pivot_owner_for_index(
            matter_owner_pivot_user_id, pivot_users,
        )
        if matter_owner_pinyin is None:
            raise PublishError(
                f"matter owner not found: {matter_owner_pivot_user_id}"
            )
    ...
    owner_notify_open_id = (
        _get_feishu_open_id_for_user(matter_owner_pivot_user_id, bindings)
        if matter_owner_pivot_user_id else None
...
def publish_matter_owner_change(
    ...
    *,
    ...
    target_user = (
        pivot_users.get(to_owner_pivot_user_id)
        or pivot_users.get_by_pinyin(to_owner_pivot_user_id)
        if pivot_users is not None
        else None
    )
    if target_user is None or target_user.status != "active" or not target_user.pinyin:
        raise PublishError(f"owner_unknown:{to_owner_pivot_user_id}")
